...
describe('MaxLengthValidator', () => {
  ...
  describe('#validate', () => {
    ...
    it('Should result in error if value length is higher than 5', async () => {
      const result = await validatorRunner.runValidations(
        'this text is higher than 5 positions',
        undefined,
        [details]
      );
      expect(result).to.be.length(1);
    ...
    it('Should result in error if value is an array with length higher than 5', async () => {
      const result = await validatorRunner.runValidations([1, 2, 3, 4, 5, 6], undefined, [details]);
      expect(result).to.be.length(1);
    });
